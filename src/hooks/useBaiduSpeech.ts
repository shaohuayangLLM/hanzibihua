...
export const useBaiduSpeech = (options: BaiduSpeechOptions = {}) => {
  ...
  // Get Supabase config from env or props
  const supabaseUrl = options.supabaseUrl || import.meta.env.VITE_SUPABASE_URL;
  const supabaseAnonKey = options.supabaseAnonKey || import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY;
  ...
  useEffect(() => {
    ...
    setIsReady(!!supabaseUrl && !!supabaseAnonKey);
      ...
      mediaRecorder.onstop = async () => {
        ...
        reader.onloadend = async () => {
          try {
            ...
            const response = await fetch(`${supabaseUrl}/functions/v1/baidu-speech`, {
              method: 'POST',
              headers: {
  ...
  }, [isSupported, isReady, supabaseUrl, supabaseAnonKey]);
