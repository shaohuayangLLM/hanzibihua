...
const AntonymSynonymPractice = () => {
  ...
  const selectAnswer = (word: string) => {
    ...
    const isCorrect = word === currentExercise.match;
    setAnswerState({ selected: word, isCorrect });
    ...
    if (isCorrect) {
      ...
      toast.error(`❌ 不对哦，正确答案是"${currentExercise.match}"`);
      setCompletedCount(prev => prev + 1);
    }
  };
  ...
  const getTypeInfo = (type: string) => {
    return WORD_TYPES.find(t => t.value === type) || WORD_TYPES[0];
  };
      ...
      <div className="min-h-screen bg-background">
        <header className="w-full py-6 px-4 border-b border-border/50 bg-card/50 backdrop-blur-sm">
          <div className="max-w-4xl mx-auto flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold">反义词/近义词</h1>
          </div>
        </header>
        ...
        <main className="max-w-4xl mx-auto px-4 py-8">
          <Card className="p-8">
            <div className="text-center space-y-6">
              ...
              <h2 className="text-3xl font-bold">反义词/近义词练习</h2>

              <div className="text-left space-y-4 max-w-md mx-auto">
                <p className="text-muted-foreground">
                  反义词是意思相反的词，近义词是意思相近的词。掌握它们能让你更好地理解和运用词语。
                </p>

                <div className="bg-secondary p-4 rounded-lg space-y-2">
                  <p className="font-semibold">练习规则：</p>
                  <ul className="text-sm text-muted-foreground space-y-1 list-disc list-inside">
                    <li>共 {WORD_RELATIONS.length} 道练习题</li>
                    <li>包含反义词和近义词两种类型</li>
                    <li>可按分类和类型筛选</li>
                    <li>每题 4 个选项，选择正确的词</li>
                  </ul>
                </div>
  ...
  // 练习页面
  const typeInfo = getTypeInfo(currentExercise.type);
        ...
        <Card className="p-6">
          <div className="text-center space-y-4">
            ...
            <div className="flex justify-center gap-2">
              <div
                className={`px-3 py-0.5 rounded-full text-white text-xs font-medium ${typeInfo.color}`}
              >
                {typeInfo.label}
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 max-w-md mx-auto">
              {currentExercise.options.map((option) => {
                ...
                const isCorrectOption = option === currentExercise.match;
            ...
            {answerState.isCorrect !== null && (
              <div className="bg-primary/10 p-3 rounded-lg">
                <p className="text-xs text-muted-foreground mb-1">
                  {currentExercise.type === "antonym" ? "反义词是" : "近义词是"}
                </p>
                <p className="text-xl font-bold text-primary">
                  {currentExercise.match}
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {currentExercise.matchPinyin}
                </p>
              </div>
            )}
              ...
              <div className={`flex items-center justify-center gap-2 text-base ${
                ...
                {answerState.isCorrect ? (
                  ...
                  <>
                    <X className="h-5 w-5" />
                    <span>
                      正确答案是：<strong>{currentExercise.match}</strong>
                    </span>
                  </>
                )}
              </div>
          ...
          <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-2">
            {filteredExercises.map((exercise, idx) => (
              <button
                ...
                `}
              >
                <div className="text-sm font-medium">{exercise.word}</div>
                <div className={`text-[10px] ${
                  exercise.type === 'antonym' ? 'text-red-500' : 'text-green-500'
                }`}>
...
export default AntonymSynonymPractice;
